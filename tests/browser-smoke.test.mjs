...
assert.match(styles, /\.stats-panel\s*>\s*\.dao-heart-card\s*\{\s*order:\s*2/s);
assert.match(styles, /\.stats-panel\s*>\s*\.tribulation-card\s*\{\s*order:\s*3/s);
assert.match(styles, /\.stats-panel\s*>\s*header\s*\{\s*order:\s*4/s);
assert.match(styles, /\.stats-panel\s*>\s*\.vital-grid\s*\{\s*order:\s*5/s);
...
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mission-map-selector\s*\{[^}]*display:\s*flex/s);
