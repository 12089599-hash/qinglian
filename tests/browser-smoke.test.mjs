...
assert.match(styles, /body:not\(\[data-active-tab="overview"\]\)\s+\.hero\s*\{[^}]*min-height:\s*132px/s);
assert.match(styles, /body:not\(\[data-active-tab="overview"\]\)\s+\.hero canvas\s*\{[^}]*height:\s*132px/s);
assert.match(styles, /\.toast-stack\s*\{[\s\S]*bottom:\s*calc\(82px \+ env\(safe-area-inset-bottom\)\)/);
