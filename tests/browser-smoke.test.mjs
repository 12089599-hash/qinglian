...
assert.match(html, /cloudClient\.js\?v=/);
assert.match(source, /createQinglanCloudClient/);
assert.match(source, /function initHeroArtPreload/);
assert.match(source, /assets\/hero\/hero-mobile\.webp/);
assert.match(styles, /--hero-placeholder:\s*url\("assets\/hero\/hero-placeholder\.webp"\)/);
assert.match(styles, /--hero-active-art:\s*url\("assets\/hero\/hero-desktop\.webp"\)/);
assert.match(styles, /--hero-active-art:\s*url\("assets\/hero\/hero-tablet\.webp"\)/);
assert.match(styles, /--hero-active-art:\s*url\("assets\/hero\/hero-mobile\.webp"\)/);
assert.match(styles, /--icon-cultivation:\s*url\("assets\/ui-icons\/qinglan-export\/512-small-optimized\/01-cultivation-taiji\.png"\)/);
assert.match(styles, /--icon-breakthrough:\s*url\("assets\/ui-icons\/qinglan-export\/512-small-optimized\/05-breakthrough-flame\.png"\)/);
assert.match(styles, /--icon-tribulation:\s*url\("assets\/ui-icons\/qinglan-export\/512-small-optimized\/15-tribulation-talisman\.png"\)/);
assert.match(styles, /background:\s*var\(--nav-icon\) center center \/ contain no-repeat/);
assert.match(styles, /background:\s*var\(--action-icon\) center center \/ contain no-repeat/);
assert.ok(statSync(new URL('../assets/ui-icons/qinglan-export/512-small-optimized/01-cultivation-taiji.png', import.meta.url)).size > 20 * 1024);
assert.ok(statSync(new URL('../assets/ui-icons/qinglan-export/512-small-optimized/15-tribulation-talisman.png', import.meta.url)).size > 20 * 1024);
assert.ok(statSync(new URL('../assets/hero/hero-mobile.webp', import.meta.url)).size > 300 * 1024);
assert.ok(statSync(new URL('../assets/hero/hero-mobile.webp', import.meta.url)).size < 700 * 1024);
assert.ok(statSync(new URL('../assets/hero/hero-tablet.webp', import.meta.url)).size < 700 * 1024);
assert.ok(statSync(new URL('../assets/hero/hero-desktop.webp', import.meta.url)).size < 700 * 1024);
assert.ok(statSync(new URL('../assets/hero/hero-placeholder.webp', import.meta.url)).size < 8 * 1024);
assert.match(source, /data-cloud-status/);
assert.match(source, /function renderCloudPanel/);
assert.match(source, /function handleCloudLogin/);
assert.match(source, /function handleCloudRegister/);
assert.match(source, /function uploadCloudSnapshot/);
assert.match(source, /function pullCloudSnapshot/);
assert.match(source, /function refreshLeaderboard/);
assert.match(source, /function requestCloudAccountDeletion/);
assert.match(styles, /\.cloud-tools\s*\{/);
assert.match(html, /<body data-active-tab="overview" data-active-group="practice" data-access-locked="false">/);
assert.doesNotMatch(html, /data-access-gate/);
assert.doesNotMatch(html, /data-access-input type="password"/);
assert.doesNotMatch(source, /function createAccessGate/);
assert.match(source, /function isNativeAppRuntime/);
assert.match(source, /capacitor\?\.isNativePlatform\?\.\(\)/);
assert.doesNotMatch(source, /qinglan-cave-access-v1/);
...
assert.match(source, /currentBalanceVersion\s*=\s*7/);
...
assert.match(styles, /assets\/map-icons\/qinglan-mountain\.png\?v=202605120100/);
assert.match(styles, /assets\/cave-icons\/alchemy-furnace\.png\?v=202605120100/);
...
assert.match(styles, /body\[data-active-tab="overview"\]\s+\.hero\s*\{[^}]*min-height:\s*calc\(160px \+ env\(safe-area-inset-top\)\)/s);
assert.match(styles, /body\[data-active-tab="overview"\]\s+\.hero canvas\s*\{[^}]*position:\s*absolute[^}]*height:\s*100%/s);
assert.match(styles, /body\[data-active-tab="overview"\]\s+\.hero-panel\s*\{[^}]*top:\s*calc\(8px \+ env\(safe-area-inset-top\)\)/s);
assert.match(styles, /\.mobile-cultivation-hud\s*\{[^}]*display:\s*none/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-cultivation-hud\s*\{[^}]*display:\s*flex/s);
...
assert.match(styles, /\.stats-panel\s*>\s*header\s*\{\s*order:\s*1/s);
assert.match(styles, /\.stats-panel\s*>\s*\.opening-objective\s*\{\s*order:\s*2/s);
assert.match(styles, /\.stats-panel\s*>\s*\.action-row\s*\{\s*order:\s*3/s);
assert.match(styles, /\.stats-panel\s*>\s*\.foundation-hint\s*\{\s*order:\s*4/s);
assert.match(styles, /\.stats-panel\s*>\s*\.progress-plan\s*\{\s*order:\s*5/s);
assert.match(styles, /\.stats-panel\s*>\s*\.cultivation-drawer\s*\{\s*order:\s*6/s);
assert.match(styles, /\.stats-panel\s*>\s*\.resource-drawer:not\(\.state-drawer\)\s*\{\s*order:\s*7/s);
assert.match(styles, /\.stats-panel\s*>\s*\.state-drawer\s*\{\s*order:\s*8/s);
assert.match(styles, /\.stats-panel\s*>\s*\.attribute-card\s*\{\s*order:\s*8/s);
assert.match(styles, /\.stats-panel\s*>\s*\.dao-heart-card\s*\{\s*order:\s*9/s);
assert.match(styles, /\.stats-panel\s*>\s*\.tribulation-card\s*\{\s*order:\s*10/s);
...
assert.match(source, /beast-compendium/);
assert.match(source, /selectedSpiritBeastId/);
assert.match(source, /compact-system-row/);
assert.match(source, /treasure-icon-/);
assert.match(source, /镇魂铃/);
assert.match(source, /function getTreasureSources/);
...
assert.match(styles, /\.dashboard\s*\{[^}]*align-items:\s*start/s);
assert.match(styles, /\.map-challenge-strip\s*\{[^}]*grid-template-columns:\s*1fr/s);
...
assert.match(styles, /\.boss-counsel\s*\{[^}]*display:\s*none/s);
...
assert.match(html, /data-opening-objective/);
assert.match(source, /function renderOpeningObjective/);
assert.match(source, /function getOpeningObjectives/);
assert.match(source, /claimOpeningObjective\(state/);
assert.match(source, /function formatHudAction/);
assert.match(source, /refs\.hudRate\.textContent = `\$\{formatRate\(calculateQiRate\(state, Date\.now\(\)\)\)\}\/分`/);
assert.match(source, /const shouldUseInlineMobilePages = isMobileLayout\(\)/);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.hero-panel\s*\{[^}]*justify-items:\s*center/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.hero\s*\{[^}]*min-height:\s*calc\(252px \+ env\(safe-area-inset-top\)\)/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.hero canvas\s*\{[^}]*opacity:\s*1/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.next-guidance\s*\{[^}]*display:\s*none/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.opening-objective\s*\{/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.realm-line span:first-child\s*\{[^}]*font-size:\s*24px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.realm-line span:first-child\s*\{[^}]*font-weight:\s*700/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.realm-line span:last-child\s*\{[^}]*font-size:\s*14px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.progress-track::after\s*\{[^}]*font-size:\s*14px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-cultivation-hud small\s*\{[^}]*font-size:\s*13px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-cultivation-hud strong\s*\{[^}]*font-size:\s*13px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.sub-tabs button\s*\{[^}]*font-size:\s*16px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.stats-panel > header h2\s*\{[^}]*font-size:\s*15px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.stats-panel > header \.primary-button\s*\{[^}]*font-size:\s*14px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.stats-panel > \.action-row button\s*\{[^}]*min-height:\s*34px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\[data-active-tab="overview"\] \.stats-panel > \.action-row button\s*\{[^}]*font-size:\s*14px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.opening-objective strong\s*\{[^}]*font-size:\s*15px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.resource-drawer summary\s*\{[^}]*font-size:\s*16px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.cultivation-drawer summary span\s*\{[^}]*font-size:\s*14px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-detail-frame h2\s*\{[^}]*font-size:\s*22px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-detail-body \.attribute-row strong\s*\{[^}]*font-size:\s*14px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.section-tabs button\s*\{[^}]*min-height:\s*38px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.section-tabs button\s*\{[^}]*font-size:\s*16px/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-detail-dialog\[open\]\s*\{[^}]*width:\s*100vw/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-detail-dialog\[open\]\s*\{[^}]*height:\s*min\(38dvh, 360px\)/s);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.mobile-detail-dialog\[open\]\s*\{[^}]*margin:\s*auto 0 calc\(88px \+ env\(safe-area-inset-bottom\)\)/s);
...
assert.match(source, /const detailsOpen = '';/);
...
assert.match(source, /data-mission-detail=/);
assert.match(source, /data-mission-drawer=/);
assert.match(source, /function saveMissionDetails/);
assert.match(source, /function saveMissionDrawers/);
