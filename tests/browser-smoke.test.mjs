...
assert.match(styles, /body\[data-active-tab="overview"\]\s+\.hero\s*\{[^}]*min-height:\s*216px/s);
assert.match(styles, /body\[data-active-tab="overview"\]\s+\.hero canvas\s*\{[^}]*height:\s*216px/s);
...
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.progress-plan-grid\s*\{[^}]*grid-template-columns:\s*repeat\(5, minmax\(0, 1fr\)\)/);
...
assert.equal(Number(mobileDashboardPadding[1]) >= 96, true);
...
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*body\s*\{[^}]*padding-bottom:\s*calc\(78px \+ env\(safe-area-inset-bottom\)\)/);
...
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.building-grid\s*\{[^}]*display:\s*grid[^}]*grid-template-rows:\s*repeat\(2, minmax\(76px, auto\)\)/s);
...
assert.match(source, /穿戴：\$\{item\.equippedLoot\.name\}/);
assert.match(renderGearRowSource, /gear-card-brief/);
assert.match(renderGearRowSource, /gear-quick-line/);
...
assert.match(styles, /grid-template-areas:\s*"filters filters"\s*"options actions"\s*"keep actions"/);
assert.match(styles, /\.gear-subtabs\s*\{[\s\S]*?flex-wrap: nowrap/);
assert.match(styles, /\.progress-plan/);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.progress-plan-grid\s*\{[^}]*display:\s*grid[^}]*grid-template-columns:\s*repeat\(5, minmax\(0, 1fr\)\)/s);
...
assert.match(styles, /\.loot-filters\s*\{[^}]*grid-column:\s*1 \/ -1/s);
assert.match(html, />批量分解品质</);
assert.match(html, />恢复默认</);
assert.match(styles, /@media \(max-width: 760px\)[\s\S]*\.loot-filters\s*\{[^}]*flex-wrap:\s*nowrap[^}]*overflow-x:\s*auto/s);
