...
const spiritBeastQualities = {
  ...
  function getRetiredGearSoulRefund(state) {
    ...
    const talents = normalizeLevels(state.gearSoulTalents, gearSoulTalents);
    ...
    return {
      ...
      loot: (state.lootEquipment || []).map((item) => {
        ...
        return {
          ...
          name: item.name,
          slot: item.slot,
          realmBand: lootEquipment[item.templateId]?.realmBand || '炼气',
          minRealmIndex: lootEquipment[item.templateId]?.minRealmIndex || 0,
          lootTier: lootEquipment[item.templateId]?.lootTier || 0,
  ...
  function getSpiritBeastGuidance(state) {
    ...
    if (!activeId) {
      ...
      const affordable = Object.values(spiritBeasts)
  ...
  function formatMitigationText(round) {
    if (!round?.mitigated) {
      return '';
    }
        ...
        return `
          <details class="equipment-detail-card detail-row loot-equipment-card" data-loot-detail="${item.uid}" ${openLootDetails.has(item.uid) ? 'open' : ''}>
            ...
            <div class="detail-stack">
              ...
              <small>品阶：${item.rarity?.name || '凡品'} · ${(item.variant?.affixes?.length || 1)}词条 · 器位 ${item.slotLevel || 0}/${item.slotMaxLevel || item.maxLevel}</small>
  ...
  function updateLootOrganizeButton() {
    ...
    button.textContent = !selected.length
      ? '请选择品质'
      : summary.count > 0
        ? `分解 ${summary.count}${summary.reward.bloodEssence ? ` · 血脉+${summary.reward.bloodEssence}` : summary.reward.forgingEssence ? ` · 精魄+${summary.reward.forgingEssence}` : ''}`
        : '无可分解';
    ...
    return `
      <details class="gear-set-panel compact-set-panel">
        ...
        <div class="gear-set-names">
          ${sets.map((set) => `<em class="${set.active ? 'active' : ''}">${set.name}<small>${set.matched}/${set.total}</small></em>`).join('')}
        </div>
  ...
  function getGatherQiPillBoostMultiplier(state) {
    ...
    if (realmIndex >= 27) return 1.15;
    if (realmIndex >= 18) return 1.22;
    if (realmIndex >= 9) return 1.32;
    return 1.45;
  }
  ...
  function getSpiritBeastQuality(beast) {
    return spiritBeastQualities[beast?.qualityId] || spiritBeastQualities.spirit;
  }
